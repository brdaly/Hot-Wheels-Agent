...
function ResultCard({ car, collection, onCollectionChange }: { car: CarResult; collection: CollectionControl; onCollectionChange: (value: CollectionControl) => void }) {
  ...
  const recentExactSoldComps = soldComps.filter((comp) => comp.matchQuality === "exact" && freshness(comp.soldAt, 180).status === "current");
  const soldCompCount = car.marketEvidence?.soldCompCount ?? recentExactSoldComps.length;
  ...
  const verificationChecks = unique([
    ...
    ...(car.condition.grade === "unknown" ? ["Inspect card and blister condition before choosing sealed or open handling."] : []),
    ...collectionChecks,
  ]);
  const upstreamVerify = car.recommendation.verifyFirst || `${car.decisionState ?? ""}`.toLowerCase().includes("verify");
  const verifyFirst = upstreamVerify || verificationChecks.length > 0;
  const marketGrade = soldCompCount ? car.marketEvidenceGrade || car.marketEvidence?.grade || "U" : "U";
  ...
  const displayDecision = verifyFirst ? "Verify first" : collection.state === "duplicate" ? car.score.total >= 75 ? "Purposeful duplicate" : "Skip duplicate" : car.recommendation.decision;
  const decisionCopy = verifyFirst
    ? "No buy call yet. Complete the blocking checks below before acting on the score or price signal."
  ...
  return (
    <article className={`car ${verifyFirst ? "verify-first" : "decision-ready"}`} aria-labelledby={`car-title-${cardId}`}>
      ...
      <div className="car-main">
        <div className={`decision-banner ${verifyFirst ? "verify" : "ready"}`}>
          <div><span>{verifyFirst ? "VERIFY FIRST" : "DECISION READY"}</span><b>{collectionLabel}</b></div>
          <p>{decisionCopy}</p>
        </div>
        ...
        <div className="badges"><span className={verifyFirst ? "verify" : "pass"}>{displayDecision}</span><span>Market evidence {marketGrade}</span><span className={car.recommendation.conditionGate.status}>{car.condition.grade} condition</span><span className={car.priceGate.verdict}>{pretty(car.priceGate.verdict)}</span></div>
        <p className="verdict"><b>{verifyFirst ? "No buy call yet." : `${displayDecision}.`}</b> {verifyFirst ? "The collection-fit score remains visible for comparison, but it cannot override a blocking verification gate." : `${car.recommendation.packaging}. ${car.priceGate.note}`}</p>
    ...
    <main>
      ...
      <nav aria-label="Primary navigation">
        ...
        <div className="navlinks" role="tablist" aria-label="Product sections">
          {tabs.map((item) => <button id={`tab-${item.id}`} key={item.id} type="button" role="tab" aria-selected={tab === item.id} aria-controls={`panel-${item.id}`} tabIndex={tab === item.id ? 0 : -1} onClick={() => setTab(item.id)} onKeyDown={(event) => handleTabKeyDown(event, item.id)} className={tab === item.id ? "active" : ""}>{item.label}</button>)}
        </div>
      ...
      <header id="primary-content" tabIndex={-1}>
        <div id="top"><p className="eyebrow">DALY VENTURES · {PRODUCT_NAME}</p><h1>Built for<br/>the hunt.<br/><em>Ready to verify.</em></h1><p className="lede">Turn collector evidence into an exact-release candidate, independent decision signals and a clear verification queue before any buy call.</p></div>
      ...
      <div id="panel-analyze" role="tabpanel" aria-labelledby="tab-analyze" className="tab-panel" hidden={tab !== "analyze"}>
          <section className={`owner-access ${authStatus}`} aria-labelledby="owner-access-title">
            <div>
              ...
              <h2 id="owner-access-title">{authStatus === "authenticated" ? "Owner session active" : authStatus === "checking" ? "Checking owner session…" : "Sign in to run analysis"}</h2>
              <p>Billable analysis and collection records are restricted to the invited Supabase owner account.</p>
            </div>
            {authStatus === "required" ? (
          ...
          {!result && <section className="method-grid" aria-label="Analysis method"><article><span>01</span><h3>Spot</h3><p>Read the casting, card, wheels, livery and visible chase marks.</p></article><article><span>02</span><h3>Verify</h3><p>Separate exact evidence from look-alikes, hype and incomplete IDs.</p></article><article><span>03</span><h3>Score</h3><p>Rank collection fit while keeping price and condition independent.</p></article><article><span>04</span><h3>Decide</h3><p>Check first-copy or duplicate purpose before acting on a recommendation.</p></article></section>}
      ...
      <section id="panel-hunts" role="tabpanel" aria-labelledby="tab-hunts" className="data-section chase-section tab-panel" hidden={tab !== "hunts"}><div className="section-head"><div><p className="eyebrow">2026 · ATTRIBUTED REFERENCE INDEX</p><h2>The Chase Grid</h2></div><p>Thirty Super Treasure Hunt and regular Treasure Hunt leads, organized by mainline case. Open the attributed source page before calling a chase.</p></div><div className="hunt-legend"><span><i className="super-dot"/>Super Treasure Hunt</span><span><i className="regular-dot"/>Treasure Hunt</span><small>{hunts.cases.length * 2} targets · updated {hunts.asOf}</small></div><div className="hunt-grid">{hunts.cases.map((item) => <article className="hunt-case" key={item.case}><div className="case-label"><span>MAINLINE</span><strong>CASE {item.case}</strong></div>{([ ["super", item.super], ["regular", item.treasure] ] as const).map(([kind, car]) => <a className={`hunt-car ${kind}`} href={car.sourceUrl} target="_blank" rel="noreferrer" key={car.part}><div className="hunt-image reference-only"><img src="/hunt-placeholder.svg" alt="" aria-hidden="true"/><span>{kind === "super" ? "SUPER TREASURE HUNT" : "TREASURE HUNT"}</span></div><div className="hunt-copy"><small>{car.part}</small><h3>{car.name}</h3><p>Open attributed image and release reference ↗</p></div></a>)}</article>)}</div><p className="source-note">External photographs are not copied or rehosted. Open each attributed release page, then cross-check status and placement with the <a href="https://www.hwtreasure.com/treasure-hunt-checklist/" target="_blank" rel="noreferrer">HWtreasure checklist</a> and <a href="https://orangetrackdiecast.com/2026-hot-wheels-master-list-of-all-lines/" target="_blank" rel="noreferrer">Orange Track 2026 master list</a>.</p></section>

      <section id="panel-market" role="tabpanel" aria-labelledby="tab-market" className="data-section tab-panel" hidden={tab !== "market"}><div className="section-head"><div><p className="eyebrow">UNITED STATES · {retail.asOf}</p><h2>US retail gates</h2></div><p>First-party shelf benchmarks for disciplined buying. These are retail reference points—not secondary-market valuations.</p></div><div className="snapshot-status"><span className={`freshness-chip ${retailFreshness.status}`}>{retailFreshness.label}</span><p>Recheck exact SKU, store and availability before purchase.</p></div><div className="price-grid">{retail.benchmarks.map((row) => <article key={row.category}><small>{pretty(row.category)}</small><strong>${row.normalPrice.toFixed(2)}</strong><p>{row.retailer}</p><span>Strong buy ≤ ${row.strongBuyAtOrBelow.toFixed(2)}</span><a href={row.sourceUrl} target="_blank" rel="noreferrer">Retail source ↗</a></article>)}</div><div className="notes">{retail.notes.map((note) => <p key={note}>↳ {note}</p>)}</div></section>

      <footer><div><b>{PRODUCT_NAME}</b><p>Collector intelligence, not a return forecast. Exact releases and high-value claims require verification.</p></div><div><a href="https://orangetrackdiecast.com/hot-wheels-casting-database/" target="_blank" rel="noreferrer">Orange Track Database ↗</a><a href="https://www.hwtreasure.com/" target="_blank" rel="noreferrer">HWtreasure ↗</a></div><small>A Daly Ventures project · Independent and not affiliated with or endorsed by Mattel · v{packageMetadata.version}</small></footer>
    </main>
  );
}
